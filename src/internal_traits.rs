...
pub trait UnspecAddress: Sized + Copy + Eq + Ord + std::fmt::Display + std::fmt::Debug {
	fn overflowing_next(self, prefix_len: u8) -> (Self, bool);
	fn overflowing_inc(self, prefix_len: u8, step: u128) -> (Self, bool);
	fn overflowing_prev(self, prefix_len: u8) -> (Self, bool);
	fn overflowing_dec(self, prefix_len: u8, step: u128) -> (Self, bool);
	fn overflowing_sub(self, other: Self) -> (u128, bool);
	fn prefix_match(self, prefix: Self, prefix_len: u8) -> bool;
	fn has_zero_host_part(self, prefix_len: u8) -> bool;
	fn network_address(self, prefix_len: u8) -> Self;
	fn last_address(self, prefix_len: u8) -> Self;
}

/// Only implemented for IPv4Addr and IPv6Addr, NOT IpAddr
pub trait Address: UnspecAddress {
...
impl UnspecAddress for Ipv4Addr {
	fn overflowing_next(self, prefix_len: u8) -> (Self, bool) {
	...
	fn prefix_match(self, prefix: Self, prefix_len: u8) -> bool {
		...
		(u32::from(self) & net_mask) == (u32::from(prefix) & net_mask)
	}
...
impl UnspecAddress for Ipv6Addr {
	fn overflowing_next(self, prefix_len: u8) -> (Self, bool) {
	...
	fn prefix_match(self, prefix: Self, prefix_len: u8) -> bool {
		...
		(u128::from(self) & net_mask) == (u128::from(prefix) & net_mask)
	}
...
impl UnspecAddress for IpAddr {
	fn overflowing_next(self, prefix_len: u8) -> (Self, bool) {
		match self {
			Self::V4(a) => {
				let (res, of) = a.overflowing_next(prefix_len);
				(Self::V4(res), of)
			},
			Self::V6(a) => {
				let (res, of) = a.overflowing_next(prefix_len);
	...
	fn overflowing_inc(self, prefix_len: u8, step: u128) -> (Self, bool) {
		match self {
			Self::V4(a) => {
				let (res, of) = a.overflowing_inc(prefix_len, step);
				(Self::V4(res), of)
			},
			Self::V6(a) => {
				let (res, of) = a.overflowing_inc(prefix_len, step);
	...
	fn overflowing_prev(self, prefix_len: u8) -> (Self, bool) {
		match self {
			Self::V4(a) => {
				let (res, of) = a.overflowing_prev(prefix_len);
				(Self::V4(res), of)
			},
			Self::V6(a) => {
				let (res, of) = a.overflowing_prev(prefix_len);
	...
	fn overflowing_dec(self, prefix_len: u8, step: u128) -> (Self, bool) {
		match self {
			Self::V4(a) => {
				let (res, of) = a.overflowing_dec(prefix_len, step);
				(Self::V4(res), of)
			},
			Self::V6(a) => {
				let (res, of) = a.overflowing_dec(prefix_len, step);
	...
	fn overflowing_sub(self, other: Self) -> (u128, bool) {
		match (self, other) {
			(Self::V4(this), Self::V4(other)) => this.overflowing_sub(other),
			(Self::V6(this), Self::V6(other)) => this.overflowing_sub(other),
			_ => (0, false),
		}
	}
	...
	fn prefix_match(self, prefix: Self, prefix_len: u8) -> bool {
		match (self, prefix) {
			(Self::V4(this), Self::V4(prefix)) => this.prefix_match(prefix, prefix_len),
			(Self::V6(this), Self::V6(prefix)) => this.prefix_match(prefix, prefix_len),
			_ => false,
		}
	}
	...
	fn has_zero_host_part(self, prefix_len: u8) -> bool {
		match self {
			Self::V4(a) => a.has_zero_host_part(prefix_len),
			Self::V6(a) => a.has_zero_host_part(prefix_len),
		}
	}

	fn network_address(self, prefix_len: u8) -> Self {
		match self {
			Self::V4(a) => Self::V4(a.network_address(prefix_len)),
			Self::V6(a) => Self::V6(a.network_address(prefix_len)),
		}
	}

	fn last_address(self, prefix_len: u8) -> Self {
		match self {
			Self::V4(a) => Self::V4(a.last_address(prefix_len)),
			Self::V6(a) => Self::V6(a.last_address(prefix_len)),
		}
	}
}
